...
EXP290_SOURCE = (
    PROJECT_ROOT
    / "2-code/290-single-foundation-unlabeled-ensemble-compression/src/audit_unlabeled_weight_soup.py"
)
EXP191_SOURCE = (
    PROJECT_ROOT / "2-code/191-usfm-convnext-dense-fusion/src/model.py"
)
EXP290_SHA256 = "5b4a79bde04cf2d246f2e58b7ecd87406cb54b2c519b0afa38b3bc4cc2fe05f8"
EXP191_SHA256 = "b3e007d1ae10e2a48574ad13668f007b94cd7eb3ea7b6f6fb45aff039787bbd4"
...
def load_dependencies():
    exp290 = load_locked("exp290_for_exp301", EXP290_SOURCE, EXP290_SHA256)
    exp191 = load_locked("exp191_for_exp301", EXP191_SOURCE, EXP191_SHA256)
    return exp290, exp191
...
def prepare_manifest(settings: dict[str, Any]) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    screen_path = resolve(settings["exp300_split"])
    screen_sha = sha256_file(screen_path)
    if screen_sha != str(settings["exp300_split_sha256"]):
        raise RuntimeError("Exp300 split checksum mismatch.")
    ...
    if len(screen_ids) != len(screen) or not screen_ids.issubset(
        ...
        raise RuntimeError("Exp300 split IDs are invalid for the readable manifest.")
    ...
    role_by_id = dict(
        zip(
            ...
            screen["stage0_split"].astype(str).map(
                {"train": "exp300_train", "heldout": "exp300_audit"}
            ),
        )
    )
    ...
    summary = {
        ...
        "exp300_train_images": int((frame["role"] == "exp300_train").sum()),
        "exp300_audit_images": int((frame["role"] == "exp300_audit").sum()),
...
class IndexedUnlabeledDataset(Dataset):
    def __init__(self, frame: pd.DataFrame, input_size: int, exp290: Any) -> None:
...
def indexed_loader(
    ...
    dataset = IndexedUnlabeledDataset(frame, input_size, exp290)
    return DataLoader(
        dataset,
    ...
    sources.update(
        {
            ...
            PROJECT_ROOT / "1-docs/301-full-official-unlabeled-ensemble-distillation.md",
            EXP290_SOURCE,
            EXP191_SOURCE,
            PROJECT_ROOT
