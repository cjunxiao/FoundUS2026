...
def main() -> None:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("Exp301 teacher-bank generation requires CUDA.")
    ...
    if set(point_counts) != set(settings["tasks"]):
        raise RuntimeError("Teacher task point map differs from Exp301 tasks.")
    ...
    for item, payload in zip(settings["checkpoints"], payloads):
        model = exp290.build_model(exp191, payload, payload["model_state"], device)
        audit = accumulate_teacher(
            ...
            coordinate_sums,
            coordinate_square_sums,
            f"Exp301 teacher fold {int(item['fold'])}",
        )
