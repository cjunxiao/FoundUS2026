...
class BankTargetDataset(Dataset):
    def __init__(
        ...
        self.task = str(task)
        self.base = exp290.UnlabeledDataset(self.frame, int(input_size))
...
def train_task(
    ...
    loader = task_loader(frame, task, settings, bank_dir, exp290, shuffle=True)
    ...
    for micro_index, batch in enumerate(
        tqdm(loader, desc=f"Exp301 full distillation {task}"), 1
    ):
        ...
        if not bool(torch.isfinite(loss)):
            raise RuntimeError(f"Non-finite Exp301 loss for task {task}.")
...
def evaluate(
    ...
    for task in settings["tasks"]:
        ...
        loader = task_loader(current, str(task), settings, bank_dir, exp290, shuffle=False)
        values = {"anchor_js": [], "student_js": [], "anchor_coord": [], "student_coord": []}
        for batch in tqdm(loader, desc=f"Exp301 audit {task}", leave=False):
            images = batch["image"].to(device, non_blocking=True)
            target = batch["teacher_probability"].to(device, non_blocking=True).float()
            target = target / target.sum((-2, -1), keepdim=True).clamp_min(1e-12)
            with torch.autocast(
...
def main() -> None:
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("Exp301 full distillation requires CUDA.")
    ...
    if bool(settings.get("enforce_empty_output", True)) and run_dir.exists() and any(
        ...
        raise RuntimeError(f"Exp301 run directory is not empty: {run_dir}")
    ...
    if bank_summary["status"] != "pass":
        raise RuntimeError("Exp301 teacher bank did not pass QC.")
    ...
    payloads, checkpoint_records = exp290.load_payloads(settings)
    medoid_index = next(
        index
    ...
    anchor = exp290.build_model(
        exp191, medoid_payload, medoid_payload["model_state"], device
    )
    del payloads
    ...
    if common.sha256_file(warmstart_path) != str(settings["warmstart_checkpoint_sha256"]):
        raise RuntimeError("Exp300 warm-start checkpoint checksum mismatch.")
    warmstart_info = load_warmstart(network, warmstart_path)
    frozen_hash_before = exp301_model.frozen_anchor_state_sha256(network.anchor)
    warm_aggregate, warm_rows, warm_scale_zero = evaluate(
        network, audit_frame, settings, bank_dir, exp290, device
    )
    ...
    for task in settings["tasks"]:
        ...
        history, seen = train_task(
            network, current, str(task), settings, bank_dir, exp290, device
        )
        histories.append(history)
        all_seen.extend(seen)
        torch.save(
            {
                "format": "exp301_task_boundary_recovery_v1",
    ...
    frozen_hash_after = exp301_model.frozen_anchor_state_sha256(network.anchor)
    ...
    coverage = {
        ...
        "exp300_gradient_images": int(bank_summary["coverage"]["exp300_train_images"]),
        "full_continuation_gradient_images": int(len(expected_train_ids)),
        "total_unique_gradient_images": int(
            bank_summary["coverage"]["exp300_train_images"] + len(expected_train_ids)
        ),
        "locked_non_gradient_audit_images": int(
            bank_summary["coverage"]["exp300_audit_images"]
    ...
    torch.save(
        {
            "format": "exp301_full_official_unlabeled_ensemble_distillation_v1",
            "trainable_state": network.trainable_state_dict(),
            "settings": settings,
        ...
        run_dir / "exp301_task_private_lora_correction.pt",
    )
    summary = {
