...
class TaskPrivateLoRALinear(nn.Module):
    ...
    def __init__(
        ...
        if float(dropout) != 0.0:
            raise ValueError(
                "TaskLoRA requires LoRA dropout=0 because timm BEiT reads qkv.weight directly."
            )
        self.base = base
...
class TaskPrivateUSFMLoRAStudent(nn.Module):
    """One DenseFusion graph with task-private internal USFM LoRA and copied fusion."""
