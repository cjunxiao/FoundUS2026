...
MODEL_NAME = "Exp184-USFM-ViTB16-TaskPrivate-ViTPose-Heatmap"
IDENTITY_SOURCE = PROJECT_ROOT / "2-code/159-canonical-internal-exp152/src/model.py"


def _load_identity_helpers():
    name = "exp159_identity_for_exp183"
...
def build_model(settings: dict[str, Any], task_configs: list[dict[str, Any]]) -> nn.Module:
    if int(settings.get("input_size", 256)) != 256:
        raise ValueError("Exp184 fixes input_size=256 for a 16x16 token grid and 64x64 heatmaps.")
    if int(settings.get("heatmap_size", 64)) != 64:
        raise ValueError("Exp184 fixes heatmap_size=64.")
...
def configure_trainable(network: USFMViTPose, scope: str) -> dict[str, int]:
    ...
    if scope == "decoder_only":
        ...
        raise ValueError(f"Unknown Exp184 train scope: {scope}")
