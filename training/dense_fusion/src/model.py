...
EXP161_MODEL = PROJECT_ROOT / "2-code/161-stable-internal-exp152/src/model.py"
EXP184_SRC = PROJECT_ROOT / "2-code/184-usfm-task-private-vitpose/src"
MODEL_NAME = "Exp191-USFM-ConvNeXt-Dense-Fusion"
if str(EXP184_SRC) not in sys.path:
    sys.path.insert(0, str(EXP184_SRC))
...
exp161 = _load_module("exp161_model_for_exp191", EXP161_MODEL)
exp184 = _load_module("exp184_model_for_exp191", EXP184_SRC / "model.py")

canonicalize_training_target = exp161.canonicalize_training_target
canonicalize_internal_points = exp161.canonicalize_internal_points
sort_official_vertical = exp161.sort_official_vertical
...
class FrozenDualExpertEncoder(nn.Module):
    def __init__(self, settings: dict[str, Any], task_configs: list[dict[str, Any]]):
        ...
        self.load_info = {
            ...
            "spatial_alignment": (
                "Exp161 518x518 canvas; bilinear antialiased resize to "
...
def build_model(settings: dict[str, Any], task_configs: list[dict[str, Any]]) -> nn.Module:
    if int(settings.get("input_size", 518)) != 518:
        raise ValueError("Exp191 uses the Exp161 518x518 canvas as its output frame.")
    if int(settings.get("heatmap_size", 64)) != 64:
        raise ValueError("Exp191 requires 64x64 heatmaps.")
...
def forward_train(
    ...
    output["coords_norm"] = exp184._soft_argmax(
...
def compute_loss(
    ...
    total, parts = exp184.compute_loss(output, target, task_id, phase, settings)
...
def configure_trainable(
    ...
    if str(scope) != "fusion_only":
        raise ValueError(f"Exp191 first screen supports fusion_only, got {scope}.")
    for module in (
        network.fusion,
