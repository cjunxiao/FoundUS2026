...
MODEL_NAME = "Exp161-Stable-Internal-Exp152"


def _load_exp159_model():
    name = "exp159_model_for_exp161"
    spec = importlib.util.spec_from_file_location(name, EXP159_MODEL_PATH)
    if spec is None or spec.loader is None:
        raise ImportError(EXP159_MODEL_PATH)
...
_exp159 = _load_exp159_model()
A4C_PAIRS = _exp159.A4C_PAIRS
PSAX_PAIRS = _exp159.PSAX_PAIRS
OFFICIAL_VERTICAL_TASKS = _exp159.OFFICIAL_VERTICAL_TASKS
PSAXPairAwareHead = _exp159.PSAXPairAwareHead


def build_model(settings: dict[str, Any], task_configs: list[dict[str, Any]]) -> nn.Module:
    return _exp159.build_model(settings, task_configs)


def canonicalize_internal_points(points: torch.Tensor, task_id: str) -> torch.Tensor:
    return _exp159.canonicalize_internal_points(points, task_id)
...
def canonicalize_training_target(
    ...
    return _exp159.canonicalize_training_target(target, task_id, settings)
...
def sort_official_vertical(
    ...
    return _exp159.sort_official_vertical(points, task_id, settings)
...
def forward_train(
    ...
    return _exp159.forward_train(model, images, task_id, settings)
...
def forward_inference(
    ...
    return _exp159.forward_inference(
        model,
        images,
        task_id,
        phase_or_settings,
...
def endpoint_identity_policy(settings: dict[str, Any] | None = None) -> dict[str, Any]:
    policy = _exp159.endpoint_identity_policy(settings)
    policy.update(
        {
