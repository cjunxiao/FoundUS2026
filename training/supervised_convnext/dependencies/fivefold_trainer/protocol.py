...
def load_exp148_fold(
    ...
    if missing:
        raise ValueError(f"Exp157 protocol manifest missing columns: {sorted(missing)}")
    merged = full.merge(
    ...
    if merged["fold"].isna().any():
        raise RuntimeError("Exp157 protocol does not cover every selected canonical row.")
    mismatch = merged["row_id_current"].astype(str) != merged["row_id"].astype(str)
    if mismatch.any():
        raise RuntimeError(f"Exp157 protocol row checksum mismatch: {int(mismatch.sum())} rows")
    ...
    if overlap:
        raise RuntimeError(f"Exp157 train/validation group leakage: {sorted(overlap)[:10]}")
    ...
    if int((excluded["excluded_reason"] == "administrator_fetal_femur_mirror_exclusion").sum()) != 25:
        raise RuntimeError("Exp157 must exclude exactly 25 administrator-listed fetal-femur rows.")

    active = merged[merged["train_eligible"] | merged["validation_eligible"]]
    task_configs = build_task_configs(active)
    info = {
        "protocol": "exp157_active_vertical_explicit_sequence_grouped_5fold_filtered",
        "fold": val_fold,
...
class Exp148Dataset(Dataset):
    """Exp56 image pipeline with stateless per-occurrence augmentation."""
...
def collate_exp148(items: list[dict[str, Any]]) -> dict[str, Any]:
    tasks = {str(item["task_id"]) for item in items}
    if len(tasks) != 1:
        raise RuntimeError(f"Exp148 batches must be task homogeneous, got {sorted(tasks)}")
    return {
...
class DeterministicTaskUniformBatchSampler(Sampler[list[tuple[int, int]]]):
    """Match Exp56 task-uniform sampling while making repeats independently augmented."""

    def __init__(self, dataset: Exp148Dataset, batch_size: int, seed: int, steps: int | None = None):
...
class SequentialTaskBatchSampler(Sampler[list[int]]):
    def __init__(self, dataset: Exp148Dataset, batch_size: int):
