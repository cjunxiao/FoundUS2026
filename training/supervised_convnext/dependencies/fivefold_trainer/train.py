...
import model as grouped_fold_model
...
from protocol import (
    ...
    collate_grouped_fold,
    load_grouped_fold_fold,
    phase_frame,
    resolve_path,
    sha256_file,
...
DEFAULTS: dict[str, Any] = {
    "train_csv_dir": "data/labeled_csv",
    "protocol_manifest": "data/grouped_5fold_manifest.csv",
    ...
    "encoder_weights": "weights/convnext_small.in12k_ft_in1k.safetensors",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a five-fold task-routed landmark model.")
...
def resolve_settings(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if str(settings.get("primary_selection_metric")) != "final_proxy_score":
        raise ValueError(
            "FiveFoldTrainer primary checkpoint selection is fixed to final_proxy_score "
    ...
    if folds != 5 or int(settings["fold"]) not in set(range(folds)):
        raise ValueError(f"FiveFoldTrainer requires fold 0..4, got fold={settings['fold']}, folds={folds}.")
...
def build_loader(
    ...
    dataset = GroupedFoldDataset(frame, settings, augment=train, epoch=epoch)
    ...
    if train:
        ...
        loader = DataLoader(
            ...
            collate_fn=collate_grouped_fold,
            generator=generator,
        )
...
def dry_run(
    ...
    dataset = GroupedFoldDataset(frame, settings, augment=True, epoch=1)
    sampler = DeterministicTaskUniformBatchSampler(
        dataset,
        int(settings["batch_size"]),
        int(settings["seed"]),
    ...
    payload = {
        ...
        "augmentation": "original BaselineHeatmap light intensity with stateless per-occurrence RNG",
...
def run() -> None:
    ...
    train, validation, task_configs, split_info = load_grouped_fold_fold(settings)
    ...
    network = grouped_fold_model.build_model(settings, task_configs).to(device)
    initialization = load_initial_checkpoint(network, settings)
    write_json(
        run_dir / "model_metadata.json",
        {
            "model_name": grouped_fold_model.MODEL_NAME,
    ...
    if args.smoke_forward:
        phase = dict(settings["phases"][0])
        scope_audit = grouped_fold_model.configure_trainable(network, str(phase.get("train_scope", "all")))
        grouped_fold_model.set_train_mode(network, str(phase.get("train_scope", "all")))
        frame = phase_frame(train, phase)
        ...
        dataset = GroupedFoldDataset(sample, settings, augment=True, epoch=1)
        batch = collate_grouped_fold([dataset[(index, index)] for index in range(len(dataset))])
        ...
        optimizer = torch.optim.AdamW(
            grouped_fold_model.optimizer_groups(network, phase),
            weight_decay=float(settings["weight_decay"]),
        )
        output = grouped_fold_model.forward_train(network, images, task, smoke_settings)
        loss, parts = grouped_fold_model.compute_loss(output, target, task, phase, smoke_settings)
        ...
        payload = {
            ...
            "trainable_parameter_names": grouped_fold_model.trainable_parameter_names(network),
        }
    ...
    for epoch in range(start_epoch, total_epochs + 1):
        ...
        if phase_index != current_phase_index:
            scope_audit = grouped_fold_model.configure_trainable(network, scope)
            optimizer = torch.optim.AdamW(
                grouped_fold_model.optimizer_groups(network, phase),
        ...
        grouped_fold_model.set_train_mode(network, scope)
        ...
        for batch in tqdm(train_loader, desc=f"[Train {epoch:03d}/{total_epochs}]", leave=False):
            ...
            with torch.amp.autocast(
                ...
                output = grouped_fold_model.forward_train(network, images, task, epoch_settings)
                loss, parts = grouped_fold_model.compute_loss(
