...
BASELINE_HEATMAP_MODEL_PATH = Path(__file__).resolve().parents[1] / "baseline_heatmap/model.py"
MODEL_NAME = "PSAXPairHead-BaselineHeatmap-PSAX-Pair-Aware-Heatmap"
PSAX_PAIRS = ((0, 1), (2, 3))


def _load_baseline_heatmap_model():
    name = "baseline_heatmap_model_for_psax_pair_head"
    spec = importlib.util.spec_from_file_location(name, BASELINE_HEATMAP_MODEL_PATH)
    if spec is None or spec.loader is None:
        raise ImportError(BASELINE_HEATMAP_MODEL_PATH)
...
_baseline_heatmap = _load_baseline_heatmap_model()
...
class PSAXPairHeadModel(nn.Module):
    def __init__(self, settings: dict[str, Any], task_configs: list[dict[str, Any]]):
        ...
        baseline = _baseline_heatmap.build_model(baseline_settings, task_configs)
        if "PSAX" not in baseline.heads:
            raise ValueError("PSAXPairHead requires a PSAX task head.")
...
def build_model(settings: dict[str, Any], task_configs: list[dict[str, Any]]) -> nn.Module:
    return PSAXPairHeadModel(settings, task_configs)
...
def configure_trainable(model: nn.Module, scope: str) -> dict[str, int]:
    if str(scope) != "all":
        raise ValueError("PSAXPairHead BaselineHeatmap stage only supports train_scope=all.")
